...
let mapper = Parsetree.{
  ...
  expr: fun mapper expr => {
    ...
    | Pexp_sequence {pexp_desc: Pexp_extension ({txt: "map" | "await_wrap"}, contents), pexp_loc} next
    | Pexp_sequence {pexp_desc: Pexp_extension ({txt: "bind" | "await"}, contents), pexp_loc, pexp_attributes: [({txt: "wrap"}, _), ..._]} next
    => {
        let (pat, expr) = process_let contents pexp_loc;
        [%expr Let_syntax.map [%e expr] f::(fun [%p pat] => [%e mapper.expr mapper next]) ]
    }
    | Pexp_extension ({txt: "map" | "await_wrap"}, contents) => {
      let (pat, expr) = process_let contents expr.pexp_loc;
      [%expr Let_syntax.map [%e expr] f::(fun [%p pat] => ()) ];
    }

    | Pexp_sequence {pexp_desc: Pexp_extension ({txt: "bind" | "await"}, contents), pexp_loc} next => {
        let (pat, expr) = process_let contents pexp_loc;
        [%expr Let_syntax.bind [%e expr] f::(fun [%p pat] => [%e mapper.expr mapper next]) ]
    }
    | Pexp_extension ({txt: "bind" | "await"}, contents) => {
      let (pat, expr) = process_let contents expr.pexp_loc;
      [%expr Let_syntax.bind [%e expr] f::(fun [%p pat] => ()) ];
    }


    | Pexp_sequence {pexp_desc: Pexp_extension ({txt: "consume"}, contents), pexp_loc, pexp_attributes} next => {
        let (pat, expr) = process_let contents pexp_loc;
        let thenn = switch pexp_attributes {
        | [({txt: "then"}, contents)] => getExpr contents pexp_loc
        | _ => [%expr ()]
        };
        [%expr {Let_syntax.consume [%e expr] f::(fun [%p pat] => [%e mapper.expr mapper next]); [%e thenn]} ]
    }
    | Pexp_extension ({txt: "consume"}, contents) => {
        let (pat, expr) = process_let contents expr.pexp_loc;
        let thenn = switch expr.pexp_attributes {
        | [({txt: "then"}, contents)] => getExpr contents expr.pexp_loc
        | _ => [%expr ()]
        };
        [%expr {Let_syntax.consume [%e expr] f::(fun [%p pat] => ()); [%e thenn]} ]
    }
