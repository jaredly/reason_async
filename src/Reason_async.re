...
module NodeContinuations = {
  ...
  let map = fun work f::use fin => work (fun result => fin (Ok (use result)));
  let bind = fun work f::use fin => work (fun result => (use result) fin);
  let consume = fun work f::use => work use;
...
module Option/*: MonadThing with type t 'a = option 'a */ = {
  type t 'a = option 'a;
  let return x => Some x;
  let map value f::use => switch value {
    | Some x => Some (use x)
    | None => None
  };
  let bind value f::use => switch value {
    | Some x => use x
    | None => None
  };
  let consume value f::use => switch value {
    | Some x => use x
    | None => ()
  };

  let join2 one two => switch one {
    | None => None
    | Some one => switch two {
      | None => None
...
module Results = {
  ...
  let unwrap = fun value map other_return => switch value {
    | Ok x => map x f::return
